...
<section class="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('job-seeker.profile.update') }}" class="mt-6 space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            ...
            <div>
                <label for="age" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Age <span class="text-slate-400 dark:text-slate-500">(opt)</span></label>
                <input id="age" name="age" type="number" min="14" max="100" value="{{ old('age', $profile->age) }}"
                    class="mt-1.5 block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
            </div>
            <div>
        ...
        <div>
            <label for="qualification" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Highest qualification <span class="text-slate-400 dark:text-slate-500">(opt)</span></label>
            <select id="qualification" name="qualification"
                class="mt-1.5 block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
                <option value="">Select…</option>
                @foreach (\App\Models\JobSeekerProfile::QUALIFICATIONS as $value => $label)
                    <option value="{{ $value }}" @selected(old('qualification', $profile->qualification) === $value)>{{ $label }}</option>
                @endforeach
            </select>
        </div>

        <div>
            <label for="address" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Address <span class="text-slate-400 dark:text-slate-500">(opt)</span></label>
            <input id="address" name="address" type="text" value="{{ old('address', $profile->address) }}" placeholder="House / street / area"
                class="mt-1.5 block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
        </div>
        ...
        <div x-data="{
            ...
            <div class="mt-1.5 flex flex-wrap items-center gap-2 rounded-lg border border-slate-300 px-3 py-2.5 shadow-sm focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-200 dark:border-slate-700 dark:bg-slate-900">
                ...
                <input id="skill-input" x-ref="skillInput" type="text" x-model="draft"
                ...
                <button type="button" @click="add(); $refs.skillInput.focus()"
                    class="shrink-0 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-brand-700">
                    + Add
                </button>
            </div>
