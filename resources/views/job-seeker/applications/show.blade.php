...
<section class="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
        {{ $application->listing->category->name }} · Applied {{ $application->created_at->format('d M Y') }}
        · <a href="{{ route('careers.show', $application->listing) }}" target="_blank" rel="noopener" class="font-medium text-brand-700 underline hover:text-brand-800 dark:text-brand-400">View job posting</a>
    </p>
    ...
    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 class="text-sm font-semibold text-slate-900 dark:text-white">Job description</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {{ ucfirst(str_replace('_', ' ', $application->listing->employment_type)) }}
            @if ($application->listing->is_remote) &middot; Remote @endif
            @if ($application->listing->city) &middot; {{ $application->listing->city }} @endif
            @if ($application->listing->salary_min || $application->listing->salary_max)
                &middot; Rs. {{ number_format($application->listing->salary_min ?? 0, 0) }}@if ($application->listing->salary_max) – {{ number_format($application->listing->salary_max, 0) }}@endif
            @endif
        </p>
        <p class="mt-3 whitespace-pre-line text-sm leading-relaxed text-slate-600 dark:text-slate-400">{{ $application->listing->description }}</p>

        @if ($application->listing->requirements)
            <h2 class="mt-5 text-sm font-semibold text-slate-900 dark:text-white">Requirements</h2>
            <p class="mt-2 whitespace-pre-line text-sm leading-relaxed text-slate-600 dark:text-slate-400">{{ $application->listing->requirements }}</p>
        @endif
    </div>
