...
@section('title', 'Pay milestone — ' . config('app.name'))
...
<section class="mx-auto max-w-xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Your payment is held in escrow. It's released as work on your contract progresses.</p>
    ...
    <form method="POST" action="{{ route('consumer.contracts.milestones.pay.store', [$contract, $milestone]) }}" class="mt-6 space-y-5"
        ...
        <p class="text-center text-xs text-slate-400 dark:text-slate-500">By paying you agree funds are held in escrow for this contract.</p>
    </form>
</section>
@endsection
