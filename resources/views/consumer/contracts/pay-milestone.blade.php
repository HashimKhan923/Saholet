...
<section class="mx-auto max-w-xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('consumer.contracts.milestones.pay.store', [$contract, $milestone]) }}" enctype="multipart/form-data" class="mt-6 space-y-5"
        ...
        <button type="submit" class="w-full rounded-lg bg-brand-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
            Pay Rs. {{ number_format($milestone->amount, 0) }}
        </button>
        <p class="text-center text-xs text-slate-400 dark:text-slate-500">By paying you agree funds are held pending for this contract.</p>
    </form>
</section>
@endsection
