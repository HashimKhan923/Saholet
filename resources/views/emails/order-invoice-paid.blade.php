...
@php
    $methodLabel = $payment->isCash()
        ? ($order->isPickup() ? 'Pay at pickup' : 'Cash on delivery')
        : 'Bank transfer';
@endphp
...
@section('content')
    ...
    <p style="margin:0 0 24px; font-size:14px; line-height:1.6; color:#374151;">
        Hi {{ $order->consumer->name }},<br><br>
        We've confirmed your payment for order <strong>{{ $order->reference }}</strong>. Your invoice is attached to this email as a PDF for your records.
    </p>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px; border:1px solid #e2e8f0; border-radius:8px; overflow:hidden;">
        <tr>
            <td style="padding:10px 16px; background:#f8fafc; font-size:12px; color:#64748b; border-bottom:1px solid #e2e8f0;">Invoice reference</td>
            <td style="padding:10px 16px; background:#f8fafc; font-size:13px; color:#1e293b; font-weight:600; text-align:right; border-bottom:1px solid #e2e8f0;">{{ $invoice->reference }}</td>
        </tr>
        <tr>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0">
        <tr>
            <td style="border-radius:8px; background:#1a7a35;">
                <a href="{{ route('consumer.orders.show', $order) }}" style="display:inline-block; padding:12px 24px; font-size:14px; font-weight:600; color:#ffffff; text-decoration:none;">View order</a>
            </td>
        </tr>
    </table>
