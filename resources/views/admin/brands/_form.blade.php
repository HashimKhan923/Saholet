...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-5" x-data="{ submitting: false }" @submit="submitting = true">
    ...
    <div>
        <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Brand name</label>
    ...
    <x-admin.image-upload-field
        name="logo"
        label="Logo"
        help="Shown in the homepage brands strip. A transparent PNG or a logo on a white background works best (JPG, PNG or WebP, up to 4 MB)."
    ...
    <div>
        ...
        <input id="sort_order" name="sort_order" type="number" min="0" max="9999" value="{{ old('sort_order', $brand?->sort_order ?? 0) }}" required
        ...
        <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">Lower numbers appear first in the strip.</p>
    ...
    <div class="flex items-center gap-3 pt-2">
        ...
        <a href="{{ route('admin.brands.index') }}" class="rounded-lg border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">Cancel</a>
    </div>
</form>
