...
<section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-6 grid grid-cols-3 gap-4">
        <x-stat-card label="Total" :value="$counts['total']" tone="brand">
            <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="4" y="3" width="16" height="18" rx="2"/><line x1="4" y1="9" x2="20" y2="9" stroke-linecap="round"/><circle cx="8" cy="6" r="0.6" fill="currentColor"/></svg>
        </x-stat-card>
    ...
    <form method="GET" action="{{ route('admin.services.index') }}" class="mt-6 flex items-center gap-2">
        <input type="search" name="q" value="{{ $q }}" placeholder="Search services or categories…"
        ...
        @if ($q)
            <a href="{{ route('admin.services.index') }}" class="text-sm text-slate-500 hover:text-brand-600 dark:text-slate-400">Clear</a>
        @endif
    </form>
                    ...
                    <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/60">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.services.edit', $service) }}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Edit</a>
                                <x-confirm-form :action="route('admin.services.destroy', $service)" method="DELETE"
                                    ...
                                    title="Delete this service?" confirm-label="Delete" />
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-10 text-center text-slate-500 dark:text-slate-400">No services yet. Create your first one.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
