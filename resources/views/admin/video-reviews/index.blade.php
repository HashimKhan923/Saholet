...
<section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-1 font-display text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Review videos</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Video testimonials shown in a carousel on the homepage.</p>
        </div>
        <a href="{{ route('admin.video-reviews.create') }}" class="inline-flex items-center rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">+ New video</a>
    </div>
    ...
    <div class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($videoReviews as $video)
                    <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/60">
                        <td class="px-5 py-3">
                            <img src="{{ $video->thumbnail_url }}" alt="" class="h-12 w-20 rounded-lg object-cover">
                        </td>
                        <td class="px-5 py-3">
                            <p class="font-medium text-slate-900 dark:text-white">{{ $video->title }}</p>
                            @if ($video->description)
                                <p class="max-w-xs truncate text-xs text-slate-500 dark:text-slate-400">{{ $video->description }}</p>
                            @endif
                        </td>
                        <td class="px-5 py-3 text-slate-600 dark:text-slate-400">{{ $video->sort_order }}</td>
                        <td class="px-5 py-3">
                            @if ($video->is_active)
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.video-reviews.edit', $video) }}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Edit</a>
                                <x-confirm-form :action="route('admin.video-reviews.destroy', $video)" method="DELETE"
                                    ...
                                    title="Delete this video?" confirm-label="Delete" />
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-10 text-center text-slate-500 dark:text-slate-400">No review videos yet. Add your first one.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
