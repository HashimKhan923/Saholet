...
<section class="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
    <div class="mb-4 flex justify-end">
        <x-close-button href="{{ route('admin.contracts.index') }}" />
    </div>
    ...
    @if ($contract->status === 'in_progress')
        <div class="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            @if ($canMarkComplete)
                <p class="text-sm text-slate-600 dark:text-slate-400">Every item is completed or cancelled, and every milestone is released or refunded — this contract is ready to close out.</p>
                <x-confirm-form :action="route('admin.contracts.complete', $contract)"
                    button-label="Mark contract as completed" button-class="shrink-0 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-700"
                    title="Mark this contract as completed?" message="This confirms all work is done and all money has been settled. The customer will be notified."
                    confirm-label="Mark completed" />
            @else
                <p class="text-sm text-slate-500 dark:text-slate-400">This contract can be marked completed once every item is completed or cancelled, and every milestone is released or refunded.</p>
            @endif
        </div>
    @endif
    ...
    @if ($contract->isSubmitted())
        <div class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"
            ...
            <form method="POST" action="{{ route('admin.contracts.quote', $contract) }}" class="mt-6 space-y-6">
                ...
                <div>
                    <label for="admin_notes" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Notes for the consumer <span class="text-slate-400 dark:text-slate-500">(opt)</span></label>
