...
<head>
    ...
    <meta property="og:image" content="{{ asset('images/sahoulat-logo.jpg') }}">
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-700 antialiased dark:bg-slate-950 dark:text-slate-300">
    ...
    <header x-data="{
        ...
        <div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:h-[5.5rem] sm:px-6 lg:px-8">
            ...
            <a href="{{ route('home') }}" class="flex shrink-0 items-center" aria-label="{{ config('app.name') }} — home">
                <span class="rounded-2xl bg-white p-1 shadow-sm ring-1 ring-slate-200/70 transition hover:shadow-md dark:ring-slate-700">
    ...
    {{-- ========================================================= Footer --}}
    <footer id="contact" class="border-t border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-4">

                {{-- Brand column --}}
                <div class="lg:col-span-2">
                    <div class="inline-flex rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-200/70 dark:ring-slate-700">
                        <img src="{{ asset('images/sahoulat-logo.jpg') }}"
                             alt="{{ config('app.name') }} — سہولت آپ کے لیے"
                             class="h-14 w-auto"
                             width="187" height="56" loading="lazy" decoding="async">
                    </div>
                    <p class="mt-4 max-w-sm text-sm leading-relaxed text-slate-500 dark:text-slate-400">{{ __('messages.footer.blurb') }}</p>

                    {{-- Social links --}}
                    <div class="mt-5 flex items-center gap-3">
                        <a href="#" aria-label="Facebook" class="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition hover:bg-brand-50 hover:text-brand-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-brand-950/50 dark:hover:text-brand-400">
                        ...
                        <a href="#" aria-label="Instagram" class="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition hover:bg-brand-50 hover:text-brand-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-brand-950/50 dark:hover:text-brand-400">
                        ...
                        <a href="#" aria-label="WhatsApp" class="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition hover:bg-brand-50 hover:text-brand-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-brand-950/50 dark:hover:text-brand-400">
                ...
                {{-- Quick links --}}
                <div>
                    <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ __('messages.nav.services') }}</p>
                    <nav class="mt-4 space-y-2.5">
                        <a href="{{ route('services.index') }}" class="block text-sm text-slate-500 transition hover:text-brand-700 dark:text-slate-400 dark:hover:text-brand-400">{{ __('messages.landing.browse_all') }}</a>
                        <a href="{{ route('providers.index') }}" class="block text-sm text-slate-500 transition hover:text-brand-700 dark:text-slate-400 dark:hover:text-brand-400">{{ __('messages.providers.nav_label') }}</a>
                        <a href="{{ route('careers.index') }}" class="block text-sm text-slate-500 transition hover:text-brand-700 dark:text-slate-400 dark:hover:text-brand-400">{{ __('messages.nav.careers') }}</a>
                        <a href="{{ route('subscription-plans.index') }}" class="block text-sm text-slate-500 transition hover:text-brand-700 dark:text-slate-400 dark:hover:text-brand-400">{{ __('messages.nav.plans') }}</a>
                        @guest
                            <a href="{{ route('register') }}" class="block text-sm text-slate-500 transition hover:text-brand-700 dark:text-slate-400 dark:hover:text-brand-400">{{ __('messages.nav.signup') }}</a>
                        @endguest
                    </nav>
                </div>
                ...
                {{-- Contact --}}
                <div>
                    <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ __('messages.footer.get_in_touch') }}</p>
                    <div class="mt-4 space-y-2.5 text-sm text-slate-500 dark:text-slate-400">
                        <a href="mailto:support@sahoulat.pk" class="block transition hover:text-brand-700 dark:hover:text-brand-400">support@sahoulat.pk</a>
                        <a href="tel:+923000000000" class="block transition hover:text-brand-700 dark:hover:text-brand-400">+92 300 000 0000</a>
                        <p>Karachi, Pakistan</p>
                    </div>
                </div>
            </div>

            <div class="mt-12 border-t border-slate-100 pt-6 text-center text-xs text-slate-400 dark:border-slate-800">
