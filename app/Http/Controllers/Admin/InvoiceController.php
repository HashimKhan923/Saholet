...
class InvoiceController extends Controller
{
    public function index(Request $request): View
    {
        ...
        // Auto-generated (booking-completion) invoices never show here — they take up space
        // and are only ever opened from the booking's own detail page.
        $query = Invoice::with(['createdBy', 'latestEditRequest.requestedBy'])->whereNull('invoiceable_type');
    ...
    public function show(Invoice $invoice): View
    {
        ...
        $invoice->load(['items', 'latestEditRequest.requestedBy', 'latestEditRequest.reviewedBy']);
    ...
    {
        $this->ensureVisible($invoice);
        abort_unless($invoice->canBeEditedBy($request->user()), 403, 'This document is locked. Request edit access from an admin first.');
        $invoice->load('items');
    ...
    public function update(Request $request, Invoice $invoice): RedirectResponse
    {
        ...
        $invoice->update([
            ...
            'discount' => $discount,
            'total' => $total,
            // Consume the approval — the next edit needs a fresh request. No-op for
            // quotations/admins, who never rely on this flag to begin with.
    ...
    {
        $this->ensureVisible($invoice);
        abort_if($invoice->isAutoGenerated(), 403, 'Auto-generated documents cannot be deleted here.');
        $invoice->delete();

        return redirect()->route('admin.invoices.index')->with('success', 'Document deleted.');
    }

    /** Customer copy — unit price only, never the actual/cost price or profit. */
    public function download(Invoice $invoice): Response
    {
        ...
        $internal = false;
        $pdf = Pdf::loadView('invoices.document', compact('invoice', 'internal'));

        return $pdf->download("{$invoice->reference}.pdf");
    }

    /**
     * Admin copy — adds the actual/cost price per item and a total profit line.
     * Restricted to admins only (not staff), since this is internal margin data.
     */
    public function downloadInternal(Invoice $invoice): Response
    {
        abort_unless(auth()->user()->isAdmin(), 403, 'Only admins can download the internal copy.');
        $invoice->load('items');

        $internal = true;
        $pdf = Pdf::loadView('invoices.document', compact('invoice', 'internal'));

        return $pdf->download("{$invoice->reference}-internal.pdf");
    }
    ...
    private function validateData(Request $request): array
    {
        $data = $request->validate([
            ...
            'items.*.actual_price' => ['nullable', 'numeric', 'min:0', 'max:9999999'],
        ]);
