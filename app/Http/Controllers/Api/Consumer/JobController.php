...
class JobController extends Controller
{
    ...
    /**
     * Post a job for bids. Body: service_id, description, budget?, preferred_date?,
     * address, latitude?, longitude?, city, photos[]? (multipart image uploads, max 5).
     */
    public function store(Request $request): JsonResponse
    {
        ...
        $job->load(['service.category', 'photos']);

        return response()->json(['job' => new JobPostResource($job)], 201);
    }
    ...
    public function show(Request $request, JobPost $jobPost): JsonResponse
    {
        ...
        return response()->json(['job' => new JobPostResource($jobPost)]);
    }

    public function cancel(Request $request, JobPost $jobPost): JsonResponse
    {
        $this->authorize('cancel', $jobPost);

        if (! $jobPost->isOpen()) {
            return response()->json(['message' => 'Only open jobs can be cancelled.'], 422);
        }
    ...
    public function acceptBid(Request $request, JobPost $jobPost, Bid $bid): JsonResponse
    {
        ...
        if (! $bid->isPending()) {
            return response()->json(['message' => 'This bid can no longer be accepted.'], 422);
        }
        ...
        if ($scheduledAt->isPast()) {
            return response()->json(['message' => 'This bid\'s proposed time has passed. Please ask the provider for an updated bid or choose another.'], 422);
        }
        ...
        if ($clash) {
            return response()->json(['message' => 'The provider already has a booking at that time. Please choose another bid.'], 422);
        }
        ...
        return response()->json([
            'message' => 'Bid accepted. A confirmed booking has been created.',
            'booking' => new \App\Http\Resources\BookingResource($booking->load(['service', 'providerProfile.user'])),
        ]);
    }
